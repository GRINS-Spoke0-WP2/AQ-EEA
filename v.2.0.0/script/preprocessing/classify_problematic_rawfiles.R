...
EEAfiles <-
  list.files(path = extract_path,
             pattern = ".csv")
pol_staz_year <- foreach (i = EEAfiles, .combine = rbind) %dopar% { #i = EEAfiles[1]
  print(which(EEAfiles == i))
  source("AQ-EEA/v.2.0.0/script/functions.R")
  # i, path = extract_path
  df_EEA <- open_raw(i, extract_path)
...
save(pol_staz_year,
     file = "AQ-EEA/v.2.0.0/data/preprocessing/list_raw_files.Rdata")

duplicated_dates <-
  pol_staz_year$name_file[pol_staz_year$duplicated_date == "yes" |
                            pol_staz_year$duplicated_hour == "yes"]
save(duplicated_dates, file = "AQ-EEA/v.2.0.0/data/preprocessing/duplicated_rawfiles.Rdata")

double_stations <-
  pol_staz_year[duplicated(pol_staz_year[, c(1:3)]),]
save(double_stations,
     file = "AQ-EEA/v.2.0.0/data/preprocessing/double_stations.Rdata")
...
for (i in 1:nrow(problematic_stations)) {
  ...
  save(removing_files,file="AQ-EEA/v.2.0.0/data/preprocessing/removing_files.Rdata")
  save(complementary_files,file="AQ-EEA/v.2.0.0/data/preprocessing/complementary_files.Rdata")
