...
```
The process makes extensive use of the functions contained in the [`functions.R`](script/functions.R) script.
...
From the script `EEA_download.R` raw data are collected in *csv* files in the [raw folder](data/raw), each of them corresponding to one station, one pollutant, one year. 
...
Within the raw files, there are some issues, e.g. duplicated measurements (same hours), or two different raw *csv* files contain the same station for the same pollutant and the same period. An automatic procedure address these problems: the script [`classify_problematic_rawfiles.R`](script/preprocessing/classify_problematic_rawfiles.R), described in the [Figure 2.1.1](#class_raw), makes the detection and classification of each case. The output of this procedure are in the folder [processing](data/processing): 
...
![thr_hist](plot/preprocessing/anomalies/hist_PM2.5_fix_3_thr.png) ![thr_hist](plot/preprocessing/anomalies/hist_PM2.5_fix_4_thr.png)
